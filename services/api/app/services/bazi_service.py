...
class BaziService:
    ...
    def _calculate_bazi(self, birth: datetime) -> BaziChart:
        ...
        # 月柱计算：月干 = (年干序号 * 2 + 月份) % 10
        # 这是一个简化算法，实际需要考虑节气
        month_offset = (STEM_INDEX[year_gan] * 2 + birth.month) % 10
        month_gan = HEAVENLY_STEMS[month_offset]
        month_zhi = EARTHLY_BRANCHES[(birth.month - 1) % 12]
    ...
    def _calculate_day_bazi(self, birth: datetime) -> tuple[str, str]:
        ...
        这是一个近似算法，对于大多数应用场景已经足够准确。
        """
        from datetime import date

        # 计算儒略日
        # 使用公式：JD = (1461 * (Y + 4800 + (M - 14) / 12)) / 4 + (367 * (M - 2 - 12 * ((M - 14) / 12))) / 12 - (3 * ((Y + 4900 + (M - 14) / 12) / 100)) / 4 + D - 32075
        y = birth.year
        m = birth.month
        d = birth.day
        ...
        # 日干支计算（以1984年1月1日甲子日为参考）
        # 1984年1月1日的儒略日约为2445705
        base_jd = 2445705
        offset = (jd - base_jd) % 60  # 60是甲子循环
