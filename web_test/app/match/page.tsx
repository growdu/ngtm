"use client";

import { useState } from "react";
import { AppShell } from "../components/Navigation";
import { matchResults } from "../data/mockData";
import styles from "./match.module.css";

export default function MatchPage() {
  const [selectedRank, setSelectedRank] = useState(1);
  const currentMatch = matchResults.find((m) => m.rank === selectedRank) || matchResults[0];

  return (
    <AppShell>
      <div className={styles.page}>
        <div className={styles.container}>
          {/* Header */}
          <div className={styles.header}>
            <h1 className={styles.title}>历史人物匹配</h1>
            <p className={styles.subtitle}>
              基于当前画像，系统为你匹配了最相似的历史人物
            </p>
          </div>

          <div className={styles.content}>
            {/* 左侧主卡片 */}
            <div className={styles.mainSection}>
              <div className={`${styles.card} ${styles.mainCard}`}>
                <div className={styles.mainCardHeader}>
                  <span className={styles.mainCardLabel}>现在的你，最像：</span>
                </div>
                <div className={styles.figureDisplay}>
                  <div className={styles.figureAvatar}>
                    <span>{currentMatch.figureName[0]}</span>
                  </div>
                  <div className={styles.figureInfo}>
                    <h2 className={styles.figureName}>{currentMatch.figureName}</h2>
                    <span className={styles.figureMeta}>
                      {currentMatch.dynasty} · {currentMatch.roleType}
                    </span>
                  </div>
                  <div className={styles.similarityScore}>
                    <span className="score-badge">
                      {Math.round(currentMatch.similarityScore * 100)}
                    </span>
                    <span className={styles.scoreLabel}>相似度</span>
                  </div>
                </div>
              </div>

              {/* 相似的地方 */}
              <div className={`${styles.card} ${styles.similarCard}`}>
                <div className="card-header">
                  <span className="card-title">相似的地方</span>
                </div>
                <ul className={styles.pointsList}>
                  {currentMatch.highlights.map((point, index) => (
                    <li key={index} className={styles.pointItem}>
                      <span className={styles.pointIcon}>✓</span>
                      <span>{point}</span>
                    </li>
                  ))}
                </ul>
              </div>

              {/* 不同的地方 */}
              <div className={`${styles.card} ${styles.differentCard}`}>
                <div className="card-header">
                  <span className="card-title">不同的地方</span>
                </div>
                <ul className={styles.pointsList}>
                  {currentMatch.differences.map((point, index) => (
                    <li key={index} className={styles.pointItem}>
                      <span className={styles.pointIconDiff}>≠</span>
                      <span>{point}</span>
                    </li>
                  ))}
                </ul>
              </div>

              {/* 人生阶段 */}
              <div className={`${styles.card} ${styles.stageCard}`}>
                <div className="card-header">
                  <span className="card-title">更像他的人生阶段</span>
                </div>
                <p className={styles.stageText}>{currentMatch.lifeStage}</p>
              </div>
            </div>

            {/* 右侧候选列表 */}
            <div className={styles.sidebar}>
              <div className={styles.candidateCard}>
                <h3 className={styles.candidateTitle}>候选人物</h3>
                <div className={styles.candidateList}>
                  {matchResults.map((match) => (
                    <div
                      key={match.rank}
                      className={`${styles.candidateItem} ${selectedRank === match.rank ? styles.active : ""}`}
                      onClick={() => setSelectedRank(match.rank)}
                    >
                      <div className={styles.candidateRank}>
                        <span className={styles.rankNumber}>{match.rank}</span>
                      </div>
                      <div className={styles.candidateInfo}>
                        <span className={styles.candidateName}>{match.figureName}</span>
                        <span className={styles.candidateMeta}>
                          {match.dynasty}
                        </span>
                      </div>
                      <div className={styles.candidateScore}>
                        <span className={styles.scoreValue}>
                          {Math.round(match.similarityScore * 100)}%
                        </span>
                      </div>
                    </div>
                  ))}
                </div>
              </div>

              <div className={styles.infoCard}>
                <h3 className={styles.infoTitle}>匹配说明</h3>
                <p className={styles.infoText}>
                  历史人物匹配基于你的画像维度（性格、能力、关系模式、命运节奏）与历史人物原型进行多维匹配。
                </p>
                <p className={styles.infoText}>
                  匹配结果会随着你的画像演进而变化。
                </p>
              </div>

              <div className={styles.infoCard}>
                <h3 className={styles.infoTitle}>为什么重要？</h3>
                <p className={styles.infoText}>
                  了解「你像谁」可以帮助你：
                </p>
                <ul className={styles.benefitList}>
                  <li>更清晰地理解自己的行为模式</li>
                  <li>从历史人物的成功与失败中学习</li>
                  <li>预见自己可能的人生轨迹</li>
                </ul>
              </div>
            </div>
          </div>
        </div>
      </div>
    </AppShell>
  );
}