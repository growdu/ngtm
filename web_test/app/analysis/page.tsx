...
import { profileV1, baziData } from "../data/mockData";
import styles from "./analysis.module.css";

export default function AnalysisPage() {
  const uncertainties = baziData.uncertainty;
          ...
          <div className={styles.header}>
            <div className={styles.headerContent}>
              <div className={styles.versionInfo}>
                <span className="version-tag">{profileV1.version}</span>
                <span className={styles.versionLabel}>版本</span>
              </div>
              <h1 className={styles.title}>你的第一版命理画像</h1>
              <p className={styles.confidence}>
                当前结论置信度：<strong>{profileV1.summary.overallScore}%</strong>
              </p>
            </div>
          ...
          <div className={styles.content}>
            ...
            <div className={styles.mainContent}>
              ...
              <div className={`${styles.card} ${styles.baziCard}`}>
                ...
                <div className={styles.baziGrid}>
                  <div className={styles.baziItem}>
                    ...
                    <span className={styles.baziValue}>{baziData.year}</span>
                  </div>
                  <div className={styles.baziItem}>
                    <span className={styles.baziLabel}>月柱</span>
                    <span className={styles.baziValue}>{baziData.month}</span>
                  </div>
                  <div className={styles.baziItem}>
                    <span className={styles.baziLabel}>日柱</span>
                    <span className={styles.baziValue}>{baziData.day}</span>
                  </div>
                  <div className={styles.baziItem}>
                    <span className={styles.baziLabel}>时柱</span>
                    <span className={styles.baziValue}>{baziData.hour}</span>
                  </div>
                </div>
                <div className={styles.fiveElements}>
                  <h4 className={styles.fiveElementsTitle}>五行分布</h4>
                  <div className={styles.fiveElementsGrid}>
                    {Object.entries(baziData.fiveElements).map(([element, value]) => (
              ...
              <div className={`${styles.card} ${styles.inferCard}`}>
                ...
                <ul className={styles.inferList}>
                  {profileV1.summary.keywords.map((keyword, index) => (
                ...
                <p className={styles.inferDesc}>{baziData.interpretation}</p>
              </div>
            ...
            <div className={styles.sidebar}>
              ...
              <div className={styles.sidebarCard}>
                ...
                <p className={styles.sidebarText}>
                  当前置信度 {profileV1.summary.overallScore}%，系统认为画像基础框架已建立，但细节仍需校准。
                </p>
              </div>
            </div>
          </div>
